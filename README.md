...
| `gw import` | Commits the mirror's current state — whatever you last synced, with any tool — to the hidden `refs/p4gw/main` ref that tracks pristine depot state (the `origin/main` analog). Your branch is fast-forwarded when it has no local commits; `--rebase` (`-r`) replays local commits on top, and without it divergent commits are left untouched — never stomped. Like `git fetch` / `git pull --rebase`. An import that fails partway restores your branch and working tree by itself, and the next run recopies every mirror file instead of trusting file sizes and timestamps; `--full` forces that full recopy any time. |
