...
const char* kSpec =
    ...
    "\n"
    "View:\n"
    "\t//depot/project/main/... //aaron-dev/...\n"
    "\t-//depot/project/main/art/... //aaron-dev/art/...\n"
    "\t\"//depot/project/main/My Docs/...\" \"//aaron-dev/My Docs/...\"\n"
    "\t//depot/tools/bin/... //aaron-dev/tools/bin/...\n"
    "\t//depot/project/main/src/... //aaron-dev/src/.p4gw/...\n";

constexpr const char* kDepotPath = "//depot/project/main/src/...";
...
TEST(spec_field_values) {
    ...
    CHECK(p4gw::p4::specField(kSpec, "Root") == "C:\\work\\project");
...
TEST(view_parses_lines_quotes_and_exclusions) {
    ...
    CHECK(view.size() == 5);
    if (view.size() == 5) {
        CHECK(view[0].depot == "//depot/project/main/...");
        CHECK(view[0].client == "//aaron-dev/...");
        CHECK(!view[0].exclude);
        CHECK(view[1].exclude);
        CHECK(view[1].depot == "//depot/project/main/art/...");
        CHECK(view[2].depot == "//depot/project/main/My Docs/...");
...
TEST(view_check_allows_narrower_exclusion_of_a_peer_dir) {
    ...
    auto view = p4gw::p4::parseClientView(kSpec);
    view.push_back({"//depot/project/main/src/lib/linux/...",
...
TEST(view_check_allows_exclude_then_reinclude_into_mirror) {
    ...
    auto view = p4gw::p4::parseClientView(kSpec);
    view.push_back({"//depot/project/main/src/lib/public/...",
                    "//aaron-dev/src/lib/public/...", true, false});
    view.push_back({"//depot/project/main/src/lib/public/win64/...",
...
TEST(view_check_allows_excluded_subtree_with_nested_client_carveouts) {
    ...
    const std::string mirror = "//client/project/.p4gw/src/...";
    const std::string repoPrefix = "//client/project/";
    const std::vector<p4gw::p4::ViewLine> view = {
        {depot, mirror, false, false},
        {"//depot/project/main/src/lib/...", "//client/project/src/lib/...", false,
         false},
        {"//depot/project/main/src/lib/public/...",
         "//client/project/src/lib/public/...", true, false},
        {"//depot/project/main/src/lib/public/win64/...",
         "//client/project/src/lib/public/win64/...", false, false},
        {"//depot/project/main/src/lib/common/...",
         "//client/project/src/lib/common/...", true, false},
        {"//depot/project/main/src/lib/common/win64*/...",
         "//client/project/src/lib/common/win64*/...", false, false},
        {"//depot/project/main/src/thirdparty/...",
         "//client/project/src/thirdparty/...", false, false},
        {"//depot/project/main/src/devtools/...", "//client/project/src/devtools/...",
         false, false},
    };
...
TEST(view_check_flags_undeclared_inplace_carveout) {
    ...
    auto view = p4gw::p4::parseClientView(kSpec);
    view.push_back({"//depot/project/main/src/thirdparty/...",
...
TEST(view_check_still_fails_when_whole_subtree_excluded) {
    ...
    const std::vector<p4gw::p4::ViewLine> view = {
        {"//depot/project/main/...", "//aaron-dev/...", false, false},
        {"//depot/project/main/src/...", "//aaron-dev/src/.p4gw/...", false, false},
        {"//depot/project/main/src/...", "//aaron-dev/src/.p4gw/...", true, false},
    };
    const auto problems =
        p4gw::p4::checkViewMapping(view, kDepotPath, kMirrorPath, kRepoPrefix);
    CHECK(!problems.empty());
...
TEST(client_view_path_inside_root) {
    CHECK(p4gw::p4::clientViewPath("aaron-dev", "/work/project",
                                   "/work/project/.p4gw", "/...") ==
          "//aaron-dev/.p4gw/...");
    CHECK(p4gw::p4::clientViewPath("aaron-dev", "/work/project",
                                   "/work/project/a/b", "/") ==
...
TEST(client_view_path_outside_root_is_empty) {
    CHECK(p4gw::p4::clientViewPath("c", "/work/project", "/elsewhere/mirror",
                                   "/...").empty());
    // The root itself can't be a view target for the mirror either.
    CHECK(p4gw::p4::clientViewPath("c", "/work/project", "/work/project",
                                   "/...").empty());
}
...
TEST(check_spec_mapping_end_to_end) {
    const std::string spec =
        ...
        "Root:\t/work\n"
        "View:\n"
        "\t//depot/project/... //c/project/...\n"
        "\t//depot/project/src/... //c/mirror/src/...\n";
    const auto good = p4gw::p4::checkSpecMapping(
        spec, "//depot/project/src/...", "/work/project/src", "/work/mirror/src");
    ...
    const auto problems = p4gw::p4::checkSpecMapping(
        broken, "//depot/project/src/...", "/work/project/src", "/work/mirror/src");
    CHECK(!problems.empty());
}
...
TEST(check_spec_mapping_allows_mirror_inside_repo) {
    ...
    const std::string spec =
        ...
        "View:\n"
        "\t//depot/project/... //c/project/...\n"
        "\t//depot/project/src/... //c/project/src/.p4gw/...\n";
    const auto good = p4gw::p4::checkSpecMapping(
        spec, "//depot/project/src/...", "/work/project/src", "/work/project/src/.p4gw");
    for (const auto& problem : good) {
        std::printf("  unexpected problem: %s\n", problem.message.c_str());
    }
    CHECK(good.empty());
    ...
    const std::string leaks =
        ...
        "\t//depot/project/... //c/project/...\n"
        "\t//depot/project/src/... //c/project/src/.p4gw/...\n"
        "\t//depot/tools/... //c/project/src/tools/...\n";
    const auto problems = p4gw::p4::checkSpecMapping(
        leaks, "//depot/project/src/...", "/work/project/src", "/work/project/src/.p4gw");
    CHECK(problems.size() == 1);
}
...
TEST(check_spec_mapping_exempts_declared_inplace_exclude) {
    ...
    const std::string spec =
        ...
        "\t//depot/project/... //c/project/...\n"
        "\t//depot/project/src/... //c/project/src/.p4gw/...\n"
        "\t//depot/project/src/thirdparty/... //c/project/src/thirdparty/...\n";
    ...
    const std::vector<std::string> excludes{"//depot/project/src/thirdparty/..."};
...
TEST(view_check_fails_when_correct_remap_is_shadowed) {
    ...
    const std::vector<p4gw::p4::ViewLine> view = {
        {"//depot/project/main/...", "//aaron-dev/...", false, false},
        {"//depot/project/main/src/...", "//aaron-dev/src/.p4gw/...", false, false},
        {"//depot/project/main/src/...", "//aaron-dev/wrong-mirror/...", false, false},
    };
    const auto problems =
        p4gw::p4::checkViewMapping(view, kDepotPath, kMirrorPath, kRepoPrefix);
    CHECK(!problems.empty());
...
TEST(view_check_passes_when_remap_is_not_last_but_unshadowed) {
    ...
    const std::vector<p4gw::p4::ViewLine> view = {
        {"//depot/project/main/...", "//aaron-dev/...", false, false},
        {"//depot/project/main/src/...", "//aaron-dev/src/.p4gw/...", false, false},
        {"//depot/project/main/config/...", "//aaron-dev/config/.p4gw/...", false,
         false},
    };
    const auto problems =
        p4gw::p4::checkViewMapping(view, kDepotPath, kMirrorPath, kRepoPrefix);
