# What working with p4gw feels like

A walkthrough of a typical couple of days using `p4gw`, as terminal
...
```
C:\work\game\src> p4gw init --depot-path //depot/game/main/src/...
...
You're ready. Daily flow:
  p4gw sync       pick up the team's changes
  git switch -c   branch and work normally
  p4gw submit     ship a branch as a P4 changelist
```
...
up. p4gw is not involved at all during this part.
...
(If the rebase had conflicted, p4gw would stop and leave you in a normal
...
```
C:\work\game\src> p4gw submit --shelve
...
didn't change (line endings, a stray build artifact), p4gw would warn loudly
instead of shelving a 4,000-file surprise.

Review comes back clean, so:

```
C:\work\game\src> p4gw submit --update 481469 --submit
...
WARN  3 files opened in the subtree outside p4gw changelists:
